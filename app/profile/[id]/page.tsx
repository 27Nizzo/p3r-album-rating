...
export default function PublicProfilePage({
  ...
  const [activeTab, setActiveTab] = useState<"reviews" | "compendium">(
    "reviews",
  );
  ...
  const handleConfidantAction = async (
  ...
  if (!profile) {
    return (
      <main className="min-h-screen bg-persona-dark text-persona-white flex flex-col items-center justify-center gap-4 p-4 text-center">
        <p className="font-mono text-xs sm:text-sm text-persona-cyan">
          OPERATIVO NÃO ENCONTRADO NA BASE DE DADOS.
        </p>
        <Link
          href="/"
          ...
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-4 md:gap-6 min-w-0">
            ...
            <div className="min-w-0">
              <div className="inline-flex items-center gap-1.5 bg-persona-cyan text-persona-dark px-2.5 py-0.5 font-black italic text-[9px] md:text-[10px] -skew-x-12 uppercase mb-1 md:mb-2">
                <Sparkles className="w-3 h-3 skew-x-12" /> PUBLIC OPERATIVE
                DOSSIER
              </div>
            ...
            <div className="mt-2 sm:mt-0 shrink-0 w-full sm:w-auto">
              {actionLoading ? (
                ...
                >
                  ...
                  <span className="skew-x-12 hidden group-hover:inline">
                    BREAK LINK 💔
                  </span>
                </button>
          ...
          <div className="skew-x-3 space-y-3">
            {userReviews.length === 0 ? (
              ...
              userReviews.map((rev) => (
                <div
                  key={rev.id}
                  className="bg-persona-dark/90 border border-persona-cyan/40 p-3 md:p-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 md:gap-4"
                >
            ...
            {favorites.length === 0 ? (
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 md:gap-4">
                {favorites.map((fav) => (
                  <div
                    key={fav.id}
                    className="bg-persona-dark/90 border border-persona-cyan/40 p-2.5 md:p-4 flex flex-col justify-between"
                  >
                    <div>
                      {/* Adicionámos max-h e controlos estritos para o telemóvel real não rebentar a imagem */}
                      <div className="w-full aspect-square bg-persona-blue/40 border border-persona-cyan/50 mb-2 overflow-hidden flex items-center justify-center relative">
                        {fav.coverUrl ? (
                          <img
                            src={fav.coverUrl}
                            alt={fav.albumTitle}
                          ...
                          <Disc className="w-8 h-8 text-persona-cyan/40" />
                        )}
                      </div>
