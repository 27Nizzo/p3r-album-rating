...
import ReviewComments from "@/components/ReviewCommets";
...
export default function Home() {
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          {[1, 2, 3, 4, 5].map((star) => (
                            ...
                            >
                              <Star
                                className={`w-8 h-8 ${star <= (hoverRating || rating) ? "text-persona-cyan fill-persona-cyan drop-shadow-[0_0_8px_rgba(0,229,255,0.8)]" : "text-persona-blue/40"}`}
                              />
                            </button>
                          ))}
                        </div>
                      ...
                      <div className="flex justify-between items-center pt-2">
                        {successMessage && (
                          <span className="text-xs font-mono text-persona-cyan flex items-center gap-1 animate-bounce">
                            <CheckCircle2 className="w-4 h-4" /> CRÍTICA
                            GUARDADA NA BD!
                          </span>
                        )}
                        <button
                ...
                {isLoadingReviews ? (
                  ...
                  <div className="bg-persona-dark/60 border border-persona-cyan/30 p-8 text-center -skew-x-6">
                    <p className="skew-x-6 font-mono text-xs text-persona-cyan/60">
                      AINDA NÃO EXISTEM CRÍTICAS NA BASE DE DADOS. SEJA O
                      PRIMEIRA A AVALIAR!
                    </p>
                  </div>
                ) : (
  ...
  >
    ...
    {/* 2. Conteúdo central e ações no fundo */}
    <div className="skew-x-6 space-y-1 w-full">
      ...
      <div className="flex justify-between items-center pt-2 text-[10px] font-mono">
        ...
        {/* Lado Direito: Likes, Data e Botão de Respostas */}
        <div className="flex items-center gap-3">
          <button
      ...
      {/* 👈 Respostas colocadas no fim do cartão (alinhadas à direita via CSS) */}
